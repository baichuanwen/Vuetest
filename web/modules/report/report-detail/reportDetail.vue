  ...
  <div class="animated fadeIn" ref="reportDetial">
      ...
      <div class="header">
        ...
        <div class="pull-right">
          <Button type="success" size="large" @click="goEdit()">编辑</Button>
          <Button type="error" size="large" @click="deleteReport()">删除</Button>
        ...
        <div class="comment-body" v-show="comments.length">
          <div class="comment-item" v-for="(comment,index) in comments" :key="index">
            ...
            <div class="content-box">
              ...
              <!--回复子级-->
              <div class="child-box" v-for="(child,iindex) in comment.replies">
                  ...
                  <div class="child-item-content">
                    ...
                    <div class="replyChild  mar-top-10 clearfix" v-show="currentChild == child.id">
                      ...
                      <p class="pull-right sub-replay">
                        ...
                        <!--<button class="btn btn-default btn-width-90" ng-click="cancelChild();childContent=''">取消</button>-->
  ...
  export default {
    ...
    methods: {
     ...
     export1 (type) {
      ...
      pre() {
        let $scope = this;
        if (this.request) this.index = this.request.listIds.indexOf(parseFloat($scope.userId));
        if (!this.request) return;
        if (this.index == 0 && this.request.page == 1) {
          this.$gcb.tooltip.open({
            icon: "warning",
            message: "无上一篇！",
            timeout: 2000
        ...
        if (this.index == 0 && this.request.page > 1) {
          this.request.page--;
          report.api.report.listSearch({
            page: this.request.page,
            pageSize: this.request.pageSize,
            startDate: this.request.condition.startDate,
            endDate: this.request.condition.endDate,
            projectId: this.request.condition.projectId,
            workReportType: this.request.condition.workReportType
          }).then((result) => {
            let temp = [];
            result.records.forEach(function (v, i) {
              temp.push(v.id);
            });
            this.request.listIds = temp;
            this.index = this.request.pageSize - 1;
            this.userId=this.request.listIds[this.index]
            $scope.detailSearch();
            $scope.comments = [];
            this.pagesInfo.page=1;
            $scope.commentSearch();
      ...
      next() {
        let $scope = this;
        if (!this.request) return;
        if (this.index == this.request.listIds.length - 1 && this.request.page == Math.ceil(this.request.total / this.request.pageSize)) {
          this.$gcb.tooltip.open({
            icon: "warning",
            message: "无下一篇！",
            timeout: 2000
      ...
      // 评论
      commentSearch() {
        this.pagesInfo.workReportId=this.userId;

      ...
      <Row class="announce-add-center">
        <Form ref="formTop" :model="reportDatas">
          ...
          <FormItem>
            ...
            <span class="text-num pull-right col-grey">{{reportDatas.plans.length||0}}/3000</span>
          </FormItem>
          <FormItem>
            ...
            <span class="text-num pull-right col-grey">{{reportDatas.records.length||0}}/3000</span>
          </FormItem>
          <FormItem>
            ...
            <span class="text-num pull-right col-grey">{{reportDatas.summaries.length||0}}/3000</span>
          </FormItem>
          <FormItem>
          ...
          <FormItem>
            <gcbUpload :attachment="attachment"  v-model="reportDatas.fileUuIds"></gcbUpload>
          </FormItem>
        </Form>
      </Row>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      getData(){
       ...
       report.api.report.detailSearch({ id:userId }).then((result)=> {
          // 项目
          if (result.project) {
            $scope.prothisject = result.project;
            $scope.projectText = result.project.projectName;
          }
          // 对谁可见
          if (result.receiveEmployees) {
            $scope.viewer = result.receiveEmployees;
            result.receiveEmployees.forEach(function(v, i) {
              $scope.viewerText += i == result.receiveEmployees.length - 1 ? v.employeeName : v.employeeName + "、";
            })
          }
          // 附件
          if (result.workReportAttachments) {
              this.attachment= result.workReportAttachments
          }
          //
          if (result.workReportType == 2) {
            $scope.reportDatas = result;
            // $scope.reportDatas.weeklyDate = result.reportDate;
            var temp = result.reportDate.split("--");
            var tempDate = "";
            for (var i = 0; i < temp.length; i++) {
              temp[i] = temp[i].replace("/", "-") + "";
              tempDate += i == 0 ? temp[i] + "至" : temp[i];
            }
            $scope.reportDatas.weeklyDate = tempDate;
            $scope.reportDatas.reportDate = null;
          } else {
            $scope.reportDatas = result;
            $scope.reportDatas.weeklyDate = null;
            if (result.workReportType == 3) {
              var date = new Date(result.calendarDate);
              date.setDate(1);
              $scope.reportDatas.reportDate = date;
            } else {
              $scope.reportDatas.reportDate = new Date(result.calendarDate);
            }
          }
       });
      } ,
      ...
      addReport() {
        ...
        report.api.report.editReport(this.reportDatas).then(res=>{
          this.$router.push({
            name: "reportList"
          })
        })

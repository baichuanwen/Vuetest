  ...
  <div class="animated fadeIn">
    ...
    <div class="header" style="border: 1px solid #EEEEEF;padding: 20px; margin-top: 30px;">
      ...
      <div class="header_item clearfix line-height-34">
        <span class="col-grey pull-left">日期：</span>
         <div class="mar-left-10 pull-left">
          <div class="header-item">
            <Select v-model="searchParams.year" class="width-160">
        ...
        <div class="mar-left-10 pull-left">
          <div class="header-item">
            <Select placeholder="全部" v-model="searchParams.month" class="width-160">
    ...
    <div class="content" v-if="!!infos.length" id="workReportTotal">
      <div class="clearfix" style="overflow: hidden;line-height: 34px">
        ...
        <div class="pull-right">
          <Button type="primary" size="large" class="btn-width-90" v-if="isHasPermission" @click="exportExcel()">导出</Button>
        </div>
      </div>
      <div class="bs-component contract-table">
        <table class="gcb-table gcb-table-hover ">
          ...
          <tr v-for="(info,$index) in infos " @click="waterClick(info.departmentID,info.departmentName)">
            ...
            <td class="text-right" :style="{'color': info.submittedCount>=0?'#666666':'red'}">{{info.submittedCount}}</td>
            <td class="text-right"  :style="{'color': info.unsubmittedCount>=0?'#666666':'red'}">{{info.unsubmittedCount}}</td>
            <td class="text-right"  :style="{'color': info.submittedRate>=0?'#666666':'red'}">{{info.submittedRate}}</td>
            <td style="text-align: right;padding-right: 15px" :style="{'color': info.rateDiff>=0?'#666666':'red'}">{{info.rateDiff}}</td>
          </tr>
          </tbody>
        </table>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //详情
       waterClick(departmentID,departmentName) {
        locals.setObject("ApplyListParams", this.searchParams);
        const params={...this.searchParams,departmentID,departmentName}
        this.$router.push({name:'wisdomWorkReportDetail', query:{params:JSON.stringify(params)}});
      },
      exportExcel(){
        wisdom.api.report.dailyReportExport(this.searchParams);
      }
    },
    created(){
      this.getYearArr();
